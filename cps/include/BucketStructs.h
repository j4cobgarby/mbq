...
// Recovers a representative priority from a bucket index: the inverse of
// prioToBucket, multiplying where the integer path shifts.
template <typename PrioTy>
static inline PrioTy bucketToPrio(int64_t bkt, uint64_t delta) {
    if constexpr (std::is_floating_point<PrioTy>::value) {
        return (PrioTy)(bkt * std::pow(2.0, (double)delta));
    } else {
        return (PrioTy)(((int64_t)bkt) << delta);
    }
}

// Maps a priority onto its bucket index. Integer priorities shift; floating
// point ones divide, since a shift is not defined for them.
template <typename PrioTy>
static inline int64_t prioToBucket(PrioTy prio, uint64_t delta) {
    if constexpr (std::is_floating_point<PrioTy>::value) {
        return (int64_t)(prio / std::pow(2.0, (double)delta));
    } else {
        return (int64_t)(prio >> delta);
    }
}
...
public:
    ...
    inline void pushBatch(uint64_t start, uint64_t size, std::vector<PQElement>& pushBuffer) {
        ...
        for (uint64_t i = 0; i < size; i++) {
            // A shift cannot bucket a floating-point priority, so divide by
            // 2^delta instead; the integer path is unchanged.
            BucketID deltaPrio = prioToBucket<PrioType>(std::get<0>(pushBuffer[idx]), delta);
